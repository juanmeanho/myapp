...
$jscomp.polyfillIsolated = function(a, b, c, d) {
    ...
    null != b && (a ? $jscomp.defineProperty($jscomp.polyfills, f, {
        ...
        writable: !0,
        value: b
    }) : b !== c && ($jscomp.propertyToPolyfillSymbol[f] = $jscomp.IS_SYMBOL_NATIVE ? $jscomp.global.Symbol(f) : $jscomp.POLYFILL_PREFIX + f,
...
$jscomp.polyfill("Map", function(a) {
    ...
    var f = function(e, g) {
        var k = g && typeof g;
        "object" == k || "function" == k ? c.has(g) ? k = c.get(g) : (k = "" + ++m,
        c.set(g, k)) : k = "p_" + g;
        var n = e.data_[k];
...
$jscomp.polyfill("Promise", function(a) {
    ...
    function c(h) {
        return h instanceof f ? h : new f(function(m, e) {
            m(h)
        }
        )
    }
    ...
    f.prototype.settle_ = function(h, m) {
        if (0 != this.state_)
            throw Error("Cannot settle(" + h + ", " + m + "): Promise already settled in state" + this.state_);
        this.state_ = h;
        this.result_ = m;
    ...
    f.prototype.executeOnSettledCallbacks_ = function() {
        if (null != this.onSettledCallbacks_) {
            for (var h = 0; h < this.onSettledCallbacks_.length; ++h)
                l.asyncExecute(this.onSettledCallbacks_[h]);
    ...
    f.race = function(h) {
        return new f(function(m, e) {
            for (var g = $jscomp.makeIterator(h), k = g.next(); !k.done; k = g.next())
                c(k.value).callWhenSettled_(m, e)
        }
        )
    }
    ;
    f.all = function(h) {
        var m = $jscomp.makeIterator(h)
          , e = m.next();
        return e.done ? c([]) : new f(function(g, k) {
            function n(r) {
                return function(t) {
                    q[r] = t;
                    p--;
            ...
            do
                q.push(void 0),
                p++,
                c(e.value).callWhenSettled_(n(q.length - 1), k),
                e = m.next();
            while (!e.done)
        }
        )
...
ABTOPhoneUA.prototype.findFreeIndex = function() {
    for (var a = 0; a < Number.MAX_SAFE_INTEGER; a++)
        if (!this.callsMapGetKey(a))
            return a
}
;
...
ABTOPhoneUA.prototype.init = function() {
    if (this.inited)
        throw "ABTOPhoneUA already inited";
    var a = this.sipProxy ? this.sipProxy : this.sipDomain;
    this.secure ? (this.sipWsUrl = "wss://" + a + ":" + this.WSPort + "/ws",
    this.transport = "WSS") : (this.sipWsUrl = "ws://" + a + ":" + this.WSPort + "/ws",
    this.transport = "WS");
...
ABTOPhoneUA.prototype.initSipRegisterStateMachine = function() {
    ...
    this.refreshRegisterTimer && (console.debug("ABTOPhoneUA(" + this.counter + "):initSipRegisterStateMachine() clearing refreshRegisterTimer timeout"),
...
ABTOPhoneUA.prototype.startLocalMedia = function(a, b, c) {
    ...
    return new Promise(function(f, l) {
        function h(e) {
            ...
            if (null != d.localAudioVideoMediaStream && d.onLocalMediaStarted)
                d.onLocalMediaStarted(e);
            f()
        }
        function m(e) {
            ...
            if ("NotAllowedError" !== e.name && "SecurityError" !== e.name && d.useVideo)
                ...
                if (d.onLocalMediaStartFailed)
                    d.onLocalMediaStartFailed(e);
                l()
            }
        }
        d.stopLocalMedia();
        d.localVideo = a;
        b ? (d.useAudio = !(null === b.audio || !1 === b.audio),
        d.useVideo = !(null === b.video || !1 === b.video)) : (d.useAudio = !0,
        d.useVideo = null !== d.localVideo,
        b = {
            audio: d.useAudio,
            video: d.useVideo
        });
        c ? h(c) : navigator.mediaDevices.getUserMedia(b).then(function(e) {
            h(e)
        }).catch(function(e) {
            m(e)
        })
...
ABTOPhoneUA.prototype.stopLocalMedia = function() {
    this.localVideo && (this.localVideo.pause(),
    this.localVideo = this.localVideo.srcObject = null);
    if (this.localAudioVideoMediaStream) {
        this.localAudioVideoMediaStream.getTracks().forEach(function(a) {
            a.stop()
        });
        if (this.onLocalMediaStopped)
            this.onLocalMediaStopped();
        this.localAudioVideoMediaStream = null
    }
}
;
ABTOPhoneUA.prototype.stopRemoteMedia = function(a) {
    null != this.remoteVideo[a] && (this.remoteVideo[a].pause(),
    this.remoteVideo[a].srcObject = null,
    this.remoteVideo[a] = null)
}
;
...
ABTOPhoneUA.prototype.inviteReferToAddress = function(a) {
    if (null != this.referToAdressString[a]) {
        ...
        this.referToAdressString[a] = null;
        a = this.invite(b);
        if (0 <= a && this.onRingingTransfer)
            this.onRingingTransfer(a, b)
    }
}
;
...
ABTOPhoneUA.prototype.processConnected = function() {
    ...
    if (0 < this.reconnectCounter)
        ...
        this.registerAfterInit && (this.register(),
        this.registerAfterInit = !1)
    }
}
;
...
ABTOPhoneUA.prototype.processResponse = function(a) {
    ...
    var b = a.getResponse();
    if ("REGISTER" == b.getCSeq().getMethod())
        this.handleStateMachineRegisterResponseEvent(a);
    else if ("MESSAGE" == b.getCSeq().getMethod())
        this.handleStateMachineMessageResponseEvent(a);
    else {
        var c = this.determineInterlocuterFromMessage(b);
        "REFER" == b.getCSeq().getMethod() ? this.handleStateMachineReferResponseEvent(c, a) : "NOTIFY" == b.getCSeq().getMethod() ? this.handleStateMachineNotifyResponseEvent(c, a) : "INFO" == b.getCSeq().getMethod() ? this.handleStateMachineInfoResponseEvent(c, a) : this.invitingState[c] != this.INVITING_INITIAL_STATE ? this.handleStateMachineInvitingResponseEvent(c, a) : this.invitedState[c] != this.INVITED_INITIAL_STATE ? this.handleStateMachineInvitedResponseEvent(c, a) : console.debug("ABTOPhoneUA(" + this.counter + "):processResponse(): response ignored")
    }
}
;
...
ABTOPhoneUA.prototype.processRequest = function(a) {
    ...
    if ("INVITE" == c) {
        ...
        this.invitingState[d] != this.INVITING_INITIAL_STATE ? this.handleStateMachineInvitingRequestEvent(d, a) : this.invitedState[d] != this.INVITED_INITIAL_STATE ? this.handleStateMachineInvitedRequestEvent(d, a) : console.warn("SimpleWebRtcSipPhone:processRequest(): request ignored");
    else if ("CANCEL" == c)
        d = this.determineInterlocuterFromMessage(b),
        1 >= this.getActiveCallsCount() && this.initConferenceStateMachine(),
        this.handleStateMachineInvitedRequestEvent(d, a);
    else if ("UPDATE" == c)
        d = this.determineInterlocuterFromMessage(b),
        this.handleStateMachineUpdateRequestEvent(d, a);
    else if ("INFO" == c)
        d = this.determineInterlocuterFromMessage(b),
        this.handleStateMachineInfoRequestEvent(d, a);
    else if ("REFER" == c) {
        d = this.determineInterlocuterFromMessage(b);
        try {
        ...
        try {
            l = b.createResponse(200, "OK"),
            l.addHeader(this.jainSipContactHeader),
            l.addHeader(this.jainSipUserAgentHeader),
            a.getServerTransaction().sendResponse(l)
        } catch (h) {
...
ABTOPhoneUA.prototype.initAndRegister = function() {
    this.inited ? this.register() : (this.registerAfterInit = !0,
    this.init())
}
;
ABTOPhoneUA.prototype.register = function() {
    ...
    if (this.registerState == this.UNREGISTERED_STATE)
        try {
            ...
            return this.initSipRegisterStateMachine(),
            console.error("ABTOPhoneUA(" + this.counter + "):register(): caught exception:" + t),
            !1
        }
    else
        return console.error("ABTOPhoneUA(" + this.counter + "):register(): bad state, action register unauthorized"),
        !1;
    return !0
}
;
ABTOPhoneUA.prototype.keepAliveRegister = function() {
    ...
    if (this.registerState == this.REGISTERED_STATE) {
        ...
        throw "ABTOPhoneUA(" + this.counter + "):keepAliveRegister(): bad state, action keep alive register unauthorized";
}
;
ABTOPhoneUA.prototype.unRegister = function() {
    ...
    if (this.registerState == this.REGISTERED_STATE) {
        ...
        null != this.refreshRegisterTimer && (console.debug("ABTOPhoneUA(" + this.counter + "):unRegister() clearing refreshRegisterTimer timeout"),
        clearTimeout(this.refreshRegisterTimer));
        ...
        this.registerState == this.UNREGISTERED_STATE ? console.warn("ABTOPhoneUA(" + this.counter + "):unRegister(): bad state, action keep alive register unauthorized") : this.unregisterPendingFlag = !0
}
;
ABTOPhoneUA.prototype.setNewViaHeader = function(a) {
    if (a instanceof SIPRequest) {
        ...
        b = new SIPResponse;
    for (var c = a.getHeaders(), d = 0; d < c.length; d++)
        b.addHeader(c[d]);
...
ABTOPhoneUA.prototype.handleStateMachineNotifyResponseEvent = function(a, b) {
    ...
    401 == d || 407 == d ? this.authenticateResponseEvent(b) : 200 == d && "refer" == b.getOriginalTransaction().getRequest().getHeader(EventHeader.prototype.NAME).getEventType() && (b = this.jainSipReferRequest[a].getHeader(ReferTo.prototype.NAME).getAddress(),
    this.referToAdressString[a] = b.encodeWithoutScheme(),
    this.referredByUri[a] = this.remoteUser[a],
    this.referLastOKResponce[a] = c)
}
;
...
ABTOPhoneUA.prototype.handleStateMachineRegisterResponseEvent = function(a) {
    ...
    if (this.registerState == this.UNREGISTERED_STATE)
        console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineRegisterResponseEvent(): bad state, SIP response ignored");
        ...
        if (200 > b)
            console.debug("ABTOPhoneUA(" + this.counter + "):handleStateMachineRegisterResponseEvent(): 1XX response ignored");
            ...
            if (200 == b) {
                ...
                if (!this.registeredFlag) {
                    ...
                    if (a = a.getContactHeader())
                        this.token = a.getParameterValue("token"),
                        console.debug("ABTOPhoneUA(" + this.counter + "):handleStateMachineRegisterResponseEvent(): got token:" + this.token);
                    if (0 < this.reconnectCounter) {
                ...
                this.unregisterPendingFlag && (this.unregisterPendingFlag = !1,
                this.unRegister())
            } else {
        ...
        if (200 > b)
            ...
            this.registeredFlag) {
                ...
                this.unInitAfterUnRegister && (this.uninit(),
                this.unInitAfterUnRegister = !1)
            }
        } else {
...
ABTOPhoneUA.prototype.call = function(a, b, c) {
    return a ? (b && (this.isConference = !0,
    0 > this.conferenceId && (this.conferenceId = Math.floor(Math.random() * Number.MAX_SAFE_INTEGER))),
    a = this.invite(a, c),
    this.isConference && (this.isConferenceInvited[a] = !0),
    a) : -1
}
;
...
ABTOPhoneUA.prototype.conference_processContacts = function(a) {
    ...
    for (var b in a)
        a.hasOwnProperty(b) && this.invite(a[b].uri, null, !1)
}
;
ABTOPhoneUA.prototype.invite = function(a, b) {
    var c = this.getCallIndex(a);
    console.debug("ABTOPhoneUA(" + this.counter + "):invite():to: " + a);
    if (this.invitingState[c] && this.invitingState[c] != this.INVITING_INITIAL_STATE)
        return console.error("ABTOPhoneUA(" + this.counter + "):call(): bad state, action call unauthorized"),
        -1;
    try {
        var d = {
            offerToReceiveAudio: this.useAudio,
            offerToReceiveVideo: this.useVideo
        };
        this.callee[c] = a;
        this.customHeaders[c] = b;
        console.log("this.callee: ", this.callee[c]);
        this.createPeerConnection(c);
        null != this.localAudioVideoMediaStream && (this.peerConnection[c].addStream(this.localAudioVideoMediaStream, null),
        this.getDTMFSender(c));
        var f = this;
        this.peerConnection[c].createOffer(function(l) {
            f.onPeerConnectionCreateOfferSuccessCallback(c, l)
        }, function(l) {
            f.onPeerConnectionCreateOfferErrorCallback(c, l)
        }, d)
    } catch (l) {
        return console.error("ABTOPhoneUA(" + this.counter + "):call(): caught exception:" + l),
        -1
    }
    return c
}
...
ABTOPhoneUA.prototype.sendInviteSipRequest = function(a, b, c) {
    ...
    try {
        ...
        if (null != b) {
            ...
            this.jainSipInvitingSentRequest[a] = this.messageFactory.createRequest(g, "INVITE", m, h, p, u, w, e);
        ...
        if (console.error("ABTOPhoneUA(" + this.counter + "):sendInviteSipRequest(): caught exception:" + B),
        this.closePeerConnection(a),
        this.inited && this.onEstablishError)
            this.onEstablishError(a, 500, "caught exception:" + B)
    }
}
;
ABTOPhoneUA.prototype.send200OKSipResponse = function(a, b) {
    ...
    try {
        ...
        throw this.closePeerConnection(a),
        console.error("ABTOPhoneUA(" + this.counter + "):send200OKSipResponse(): caught exception:" + d),
        "ABTOPhoneUA(" + this.counter + "):send200OKSipResponse(): caught exception:" + d;
    }
}
;
...
ABTOPhoneUA.prototype.mute = function() {
    this.localAudioVideoMediaStream && this.localAudioVideoMediaStream.getAudioTracks().forEach(function(a) {
        a.enabled = !1
    })
}
;
...
ABTOPhoneUA.prototype.toggleMediaStream = function(a) {
    try {
        this.paused[a] ? (this.localAudioVideoMediaStream.getAudioTracks().forEach(function(b) {}),
...
ABTOPhoneUA.prototype.transfer = function(a, b) {
    ...
    try {
        if (this.invitedState[a] != this.INVITED_ACCEPTED_STATE && this.invitingState[a] != this.INVITING_ACCEPTED_STATE)
            console.debug("ABTOPhoneUA(" + this.counter + "):transfer attempt when call not established");
        else {
        ...
        throw console.error("ABTOPhoneUA(" + this.counter + "):transfer(): caught exception:" + z),
        "ABTOPhoneUA(" + this.counter + "):transfer(): caught exception:" + z;
    }
}
;
...
ABTOPhoneUA.prototype.bye = function(a) {
    if (this.isConference)
        return this.initConferenceStateMachine(),
        this.hangupAll(),
        !1;
    ...
    if (this.invitingState[a] == this.INVITING_ACCEPTED_STATE)
        try {
            ...
            console.error("ABTOPhoneUA(" + this.counter + "):bye(): caught exception:" + d),
            console.log("ABTOPhoneUA(" + this.counter + "):bye(): caught exception:" + d),
            this.closePeerConnection(a)
        }
        ...
        try {
            ...
            console.error("ABTOPhoneUA(" + this.counter + "):cancel(): caught exception:" + d),
            this.closePeerConnection(a)
        }
    else
        console.log("ABTOPhoneUA(" + this.counter + "):bye(): bad state, action call unauthorized");
    return !0
}
;
ABTOPhoneUA.prototype.handleStateMachineInvitingResponseEvent = function(a, b) {
    ...
    if (this.invitingState[a] == this.INVITING_STATE)
        if (200 > d) {
            ...
            null != this.referredByUri[a] && (this.sendNotifyReferDoneRequest(a, c),
            this.referredByUri[a] = null);
            b = c.getContent();
            d = new RTCSessionDescription({
                type: "answer",
                sdp: b
            });
            var f = this;
            this.peerConnectionState[a] = "answer-received";
            null != b && this.peerConnection[a].setRemoteDescription(d, function() {
        ...
        if (200 > d)
            ...
            f = this,
            this.peerConnectionState[a] = "answer-received",
            this.peerConnection[a].setRemoteDescription(d, function() {
                f.onPeerConnectionSetRemoteDescriptionSuccessCallback(a)
            }, function(l) {
                f.onPeerConnectionSetRemoteDescriptionErrorCallback(l)
            }),
        ...
        this.invitingState[a] == this.INVITING_FAILED_STATE ? console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitingResponseEvent(): bad state, SIP response ignored") : this.invitingState[a] == this.INVITING_ACCEPTED_STATE ? 407 == d || 401 == d ? this.holdAuthenticateRequest(a, c) : 200 == d ? (this.sendACK(b),
        this.toggleMediaStream(a),
        this.holdState[a] = this.paused[a] ? this.HOLD_BY_ME : this.HOLD_OFF,
        this.fireOnHoldListener(a)) : console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitingResponseEvent(): bad state, SIP response ignored") : this.invitingState[a] == this.INVITING_LOCAL_HANGINGUP_STATE ? 401 == d ? "CANCEL" == b.getOriginalTransaction().getMethod() ? (c = this.headerFactory.createAuthorizationHeader(c, this.jainSipCancelRequest, this.sipPassword, this.sipLogin),
        ...
        this.jainSipCancelTransaction.sendRequest()) : (b = this.jainSipInvitingDialog[a].createRequest("BYE", this.jainSipInvitingSentRespond),
        ...
        this.closePeerConnection(a),
        this.invitedState[a] = this.INVITED_LOCAL_HANGINGUP_407_STATE) : 407 == d ? (b = this.jainSipInvitingDialog[a].createRequest("BYE"),
        d = this.sipProvider.getNewClientTransaction(b),
        c = this.headerFactory.createAuthorizationHeader(c, b, this.sipPassword, this.sipLogin),
        this.messageFactory.addHeader(b, c),
        this.jainSipInvitingDialog[a].sendRequest(d),
        this.invitingState[a] = this.INVITING_LOCAL_HANGINGUP_407_STATE) : (200 != d && console.log("Call HangUp Failed:" + c.getStatusCode() + "  " + c.getStatusLine().getReasonPhrase()),
        this.closePeerConnection(a)) : this.invitingState[a] == this.INVITING_LOCAL_HANGINGUP_407_STATE && console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitingResponseEvent(): bad state, SIP response ignored")
}
;
ABTOPhoneUA.prototype.handleStateMachineInvitingRequestEvent = function(a, b) {
    ...
    if (this.invitingState[a] == this.INVITING_STATE) {
        ...
        console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitingRequestEvent(): bad state, SIP request ignored");
    else if (this.invitingState[a] == this.INVITING_FAILED_STATE)
        console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitingRequestEvent(): bad state, SIP request ignored");
        ...
        this.invitingState[a] == this.INVITING_LOCAL_HANGINGUP_STATE ? "BYE" == d ? (c = c.createResponse(200, "OK"),
        c.addHeader(this.jainSipContactHeader),
        b.getServerTransaction().sendResponse(c),
        this.closePeerConnection(a),
        console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitingRequestEvent(): bad state, already hanging up 200 Ok anyway")) : console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitingRequestEvent(): bad state, SIP request ignored") : this.invitingState[a] == this.INVITING_LOCAL_HANGINGUP_407_STATE && console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitingRequestEvent(): bad state, SIP request ignored")
}
;
ABTOPhoneUA.prototype.handleStateMachineInvitedResponseEvent = function(a, b) {
    ...
    var c = b.getResponse()
      , d = parseInt(c.getStatusCode());
    this.invitedState[a] == this.INVITED_RINGING_STATE ? console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitedResponseEvent(): bad state, SIP response ignored") : this.invitedState[a] == this.INVITED_ACCEPTED_STATE ? 407 == d || 401 == d ? this.holdAuthenticateRequest(a, c) : 200 == d ? (this.sendACK(b),
    this.toggleMediaStream(a),
    this.holdState[a] = this.paused[a] ? this.HOLD_BY_ME : this.HOLD_OFF,
    this.fireOnHoldListener(a)) : console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitedResponseEvent(): bad state, SIP response ignored") : this.invitedState[a] == this.INVITED_LOCAL_HANGINGUP_STATE ? 407 == d || 401 == d ? (b = this.jainSipInvitedDialog[a].createRequest("BYE"),
    d = this.sipProvider.getNewClientTransaction(b),
    c = this.headerFactory.createAuthorizationHeader(c, b, this.sipPassword, this.sipLogin),
    this.messageFactory.addHeader(b, c),
    this.jainSipInvitedDialog[a].sendRequest(d),
    this.closePeerConnection(a),
    this.invitedState[a] = this.INVITED_LOCAL_HANGINGUP_407_STATE) : (200 != d && console.log("Call HangUp Failed:" + c.getStatusCode() + "  " + c.getStatusLine().getReasonPhrase()),
    this.closePeerConnection(a)) : this.invitedState[a] == this.INVITED_LOCAL_HANGINGUP_407_STATE && (200 != d && console.log("Call HangUp Failed:" + c.getStatusCode() + "  " + c.getStatusLine().getReasonPhrase()),
    this.closePeerConnection(a))
}
;
...
ABTOPhoneUA.prototype.handleStateMachineInvitedRequestEvent = function(a, b) {
    ...
    if (this.invitedState[a] == this.INVITED_INITIAL_STATE)
        if ("INVITE" == d) {
            ...
            console.debug("MEANO-ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitedRequestEvent(): sipUri.getUser()=" + this.remoteUser[a].getUser());
            if (this.inited && this.onInvited)
                try {
                    console.log("TRYYYYYYYYYYYYYYY", this.remoteUser[a].toString())
        ...
        if ("CANCEL" == d) {
            ...
            console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitedRequestEvent(): bad state, SIP request ignored");
    else if (this.invitedState[a] == this.INVITED_ACCEPTED_STATE)
        if ("BYE" == d) {
            c = c.createResponse(200, "OK");
            c.addHeader(this.jainSipContactHeader);
            ...
            "ACK" == d ? this.jainSipInvitedDialog[a] = b.getServerTransaction().getDialog() : console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitedRequestEvent(): bad state, SIP request ignored");
    else
        this.invitedState[a] == this.INVITED_LOCAL_HANGINGUP_STATE ? "BYE" == d ? (c = c.createResponse(200, "OK"),
        c.addHeader(this.jainSipContactHeader),
        b.getServerTransaction().sendResponse(c),
        this.closePeerConnection(a),
        console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitingRequestEvent(): bad state, already hanging up 200 Ok anyway")) : console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitingRequestEvent(): bad state, SIP request ignored") : this.invitedState[a] == this.INVITED_LOCAL_HANGINGUP_407_STATE && console.error("ABTOPhoneUA(" + this.counter + "):handleStateMachineInvitedRequestEvent(): bad state, SIP request ignored")
}
;
ABTOPhoneUA.prototype.handleStateMachineInfoRequestEvent = function(a, b) {
    ...
    c.addHeader(this.jainSipUserAgentHeader);
    b.getServerTransaction().sendResponse(c);
    a.hasHeader("X-Conference-ID") && this.isConference && this.conferenceId == a.getHeader("X-Conference-ID").value && (b = a.getContent(),
    this.conference_processContacts(b))
}
;
ABTOPhoneUA.prototype.handleStateMachineUpdateRequestEvent = function(a, b) {
    ...
    if (this.invitedState[a] == this.INVITED_ACCEPTED_STATE || this.invitingState[a] == this.INVITING_ACCEPTED_STATE) {
        ...
        try {
            ...
            this.peerConnection[a].setRemoteDescription(f, function() {
                l.onPeerConnectionSetRemoteDescriptionSuccessCallback(a)
            }, function(h) {
                l.onPeerConnectionSetRemoteDescriptionErrorCallback(h)
            })
        } catch (h) {
            return b = this.jainSipInvitedReceivedRequest[a].createResponse(480, "Temporarily Unavailable"),
            b.addHeader(this.jainSipContactHeader),
            b.addHeader(this.jainSipUserAgentHeader),
...
ABTOPhoneUA.prototype.createPeerConnection = function(a) {
    ...
    var b = this
      , c = {
        iceServers: this.stunServers.concat(this.turnServer)
    };
    if (this.peerConnection[a])
        try {
            this.peerConnection[a].close()
        } catch (d) {}
    this.peerConnection[a] = new RTCPeerConnection(c,{
        optional: [{
...
ABTOPhoneUA.prototype.closePeerConnection = function(a) {
    ...
    if (this.peerConnection[a]) {
        ...
        this.stopRecord(a);
        this.paused[a] && (this.toggleMediaStream(a),
        this.holdState[a] = this.HOLD_OFF);
        this.stopRemoteMedia(a);
        this.remoteAudioVideoMediaStream[a] = null;
        try {
            this.peerConnection[a].close(),
            this.peerConnection[a] = null
        } catch (b) {}
...
ABTOPhoneUA.prototype.startRecord = function(a, b) {
    ...
    MediaRecorder.isTypeSupported(c.mimeType) || (c = {
    ...
    this.mediaRecorder[a].ondataavailable = function(f) {
        f.data && 0 < f.data.size && (d.recordedBlobs[a].push(f.data),
        console.log("channel ", a, ": added chunk of ", f.data.size, " bytes"),
        d.fireOnRecordReady(a))
    }
    ;
...
ABTOPhoneUA.prototype.stopRecord = function(a) {
    null != this.mediaRecorder[a] && (this.mediaRecorder[a].stop(),
    this.mediaRecorder[a] = null)
}
;
ABTOPhoneUA.prototype.fireOnRecordReady = function(a) {
    if (null != this.recordedBlobs[a]) {
        var b = new Blob(this.recordedBlobs[a],{
            type: "video/webm"
        });
        b = window.URL.createObjectURL(b);
        if (this.onRecordReady)
            this.onRecordReady(a, b)
    }
}
;
...
ABTOPhoneUA.prototype.onPeerConnectionOnAddStreamCallback = function(a, b) {
    ...
    if (null != this.peerConnection[a]) {
        if (b = b.stream,
        this.remoteAudioVideoMediaStream[a] = b,
        null != this.remoteVideo[a] && this.playVideoStream(this.remoteVideo[a], b),
        this.doRecord && this.startRecord(a, b),
        this.inited && this.onRemoteMediaStarted)
            try {
        ...
        console.error("SimpleWebRtcSipPhone:onPeerConnectionOnAddStreamCallback(): this.peerConnection is null, bug in state machine!, bug in state machine!")
}
;
ABTOPhoneUA.prototype.onPeerConnectionOnRemoveStreamCallback = function(a, b) {
    ...
    if (null != this.peerConnection[a]) {
        if (this.stopRecord(a),
        this.remoteAudioVideoMediaStream[a] = null,
        this.stopRemoteMedia(a),
        this.inited && this.onRemoteMediaStopped)
            try {
...
ABTOPhoneUA.prototype.onPeerConnectionIceCandidateCallback = function(a, b) {
    ...
    if (null != this.peerConnection[a])
        if (console.debug("ABTOPhoneUA(" + this.counter + "):onPeerConnectionIceCandidateCallback(): this.peerConnection.readyState=" + this.peerConnection[a].readyState),
        ...
        null != b.candidate) {
            ...
            if ("preparing-offer" == this.peerConnectionState[a]) {
                if (b = this.peerConnection[a].localDescription.sdp,
                this.maxVideoBandwidth && (b = b.replace(/(m=video.*\r\nc=IN.*\r\n)/m, "$1b=AS:" + this.maxVideoBandwidth + "\r\n")),
                ...
                "preparing-answer" == this.peerConnectionState[a] || "preparing-update" == this.peerConnectionState[a] ? (b = this.peerConnection[a].localDescription.sdp,
                this.maxVideoBandwidth && (b = b.replace(/(m=video.*\r\nc=IN.*\r\n)/m, "$1b=AS:" + this.maxVideoBandwidth + "\r\n")),
                this.send200OKSipResponse(a, b),
                this.peerConnectionState[a] = "established") : "established" != this.peerConnectionState[a] && console.log("ABTOPhoneUA(" + this.counter + "):onPeerConnectionIceCandidateCallback(): RTCPeerConnection bad state!")
        }
    else
...
ABTOPhoneUA.prototype.onPeerConnectionCreateOfferSuccessCallback = function(a, b) {
    ...
    if (null != this.peerConnection[a])
        if ("new" == this.peerConnectionState[a]) {
            ...
            this.peerConnection[a].setLocalDescription(b, function() {
                c.onPeerConnectionSetLocalDescriptionSuccessCallback(a)
            }, function(d) {
                c.onPeerConnectionSetLocalDescriptionErrorCallback(a, d)
            })
        } else
            console.error("ABTOPhoneUA(" + this.counter + "):onPeerConnectionCreateOfferSuccessCallback(): RTCPeerConnection bad state!");
    else
...
ABTOPhoneUA.prototype.onPeerConnectionSetLocalDescriptionErrorCallback = function(a, b) {
    ...
    if ("preparing-update" == this.peerConnectionState[a][a])
        try {
            ...
            throw console.error("ABTOPhoneUA(" + this.counter + "):onPeerConnectionSetLocalDescriptionErrorCallback(): caught exception:" + d),
            "ABTOPhoneUA(" + this.counter + "):onPeerConnectionSetLocalDescriptionErrorCallback(): caught exception:" + d;
        }
    else
        console.log("onPeerConnectionSetLocalDescriptionErrorCallback error:" + b)
}
;
ABTOPhoneUA.prototype.onPeerConnectionCreateAnswerSuccessCallback = function(a, b) {
    ...
    if (null != this.peerConnection[a])
        if ("offer-received" == this.peerConnectionState[a]) {
            ...
            "update-received" == this.peerConnectionState[a] ? (c = this,
...
ABTOPhoneUA.prototype.onPeerConnectionSetRemoteDescriptionSuccessCallback = function(a) {
    ...
    if (null != this.peerConnection[a])
        if (console.debug("ABTOPhoneUA(" + this.counter + "):onPeerConnectionSetRemoteDescriptionSuccessCallback(): this.peerConnection[index].readyState=" + this.peerConnection[a].readyState),
        ...
        "answer-received" == this.peerConnectionState[a]) {
            if (this.peerConnectionState[a] = "established",
            this.inited) {
            ...
            var b = this;
            this.peerConnection[a].createAnswer(function(c) {
                b.onPeerConnectionCreateAnswerSuccessCallback(a, c)
            }, function(c) {
                b.onPeerConnectionCreateAnswerErrorCallback(c)
            })
        } else
            console.log("ABTOPhoneUA(" + this.counter + "):onPeerConnectionSetRemoteDescriptionSuccessCallback(): RTCPeerConnection bad state!");
    else
...
ABTOPhoneUA.prototype.onPeerConnectionSetRemoteDescriptionErrorCallback = function(a, b) {
    ...
    if (null != this.peerConnection[a])
        if (console.debug("ABTOPhoneUA(" + this.counter + "):onPeerConnectionSetRemoteDescriptionErrorCallback(): this.peerConnection.readyState=" + this.peerConnection[a].readyState),
        ...
        "answer-received" == this.peerConnectionState[a]) {
            ...
            "offer-received" != this.peerConnectionState[a] && "update-received" != this.peerConnectionState[a] || this.error(a);
    else
...
ABTOPhoneUA.prototype.accept = function(a) {
    try {
        ...
        this.peerConnection[a].setRemoteDescription(c, function() {
